...
export default function CustomerDetailPage() {
    ...
    async function load() {
      try {
        const [detailData, feedData, historyData] = await Promise.all([
          apiFetch<{ customer: Customer; eventHistory: EventBucket[] }>(
            `/api/customers/${customerId}`,
          ),
          apiFetch<ActivityItem[]>(
            `/api/customers/${customerId}/activity-feed`,
          ),
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
        <div
          className="skeleton"
          style={{ width: "200px", height: "28px", borderRadius: "6px" }}
        />
        <div
          style={{
            display: "grid",
  ...
  const displayScore =
    simResult !== null ? simResult : customer.churn_risk_probability;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div>
            <h2 style={{ fontSize: "22px", fontFamily: "monospace" }}>
              {customer.external_user_id}
            </h2>
            <span
              className={`badge ${getRiskBadgeClass(customer.risk_classification_status)}`}
              style={{ marginTop: "6px" }}
            >
          ...
          >
            {[
              ...
              },
              {
                ...
                value: formatCurrency(customer.monthly_contract_value),
                icon: <DollarSign size={16} />,
              },
              {
                label: "Last ML Scoring",
                value: formatDateTime(customer.last_computed_at),
      ...
      <div className="card" style={{ padding: "0", overflow: "hidden" }}>
        <SectionHeader
          id="shap"
          title="Top Risk Reasons (SHAP Explainability)"
        ...
        {openSection === "shap" && (
          ...
          >
            <p style={{ fontSize: "13px" }}>
              These are the top 3 model features driving this customer's churn
              score:
            </p>
                ...
                <div key={i}>
                  ...
                  >
                    <span style={{ fontWeight: 600 }}>
                      {shapLabels[f.feature] || f.feature}
                    </span>
                    <span
                      style={{
                    ...
                    >
                      {isRisk ? "+" : ""}
                      {f.shap_value.toFixed(3)} {isRisk ? "↑ risk" : "↓ risk"}
                    </span>
                  </div>
                  <div
                    style={{
      ...
      <div className="card" style={{ padding: "0", overflow: "hidden" }}>
        <SectionHeader
          id="events"
          title="8-Week Platform Usage History"
      ...
      {riskHistory.length > 0 && (
        <div className="card" style={{ padding: "0", overflow: "hidden" }}>
          <SectionHeader
            id="history"
            title="Risk Score Trend History"
            icon={<Activity size={17} />}
          />
          {openSection === "history" && (
            <div style={{ padding: "20px 24px" }}>
              <ResponsiveContainer width="100%" height={180}>
                ...
                >
                  ...
                  <Tooltip
                    ...
                    formatter={(value) => {
                      if (typeof value === "number") {
                        return [`${Math.round(value * 100)}%`, "Risk Score"];
                      }
                      return ["-", "Risk Score"];
                    }}
                  ...
                  <Line
                    type="monotone"
                    dataKey="churn_risk_probability"
                    stroke="var(--primary)"
                    strokeWidth={2}
                    dot={false}
                  />
        ...
        {openSection === "simulation" && (
          <div style={{ padding: "20px 24px" }}>
            <p style={{ fontSize: "13px", marginBottom: "20px" }}>
              Adjust the sliders below and re-run the ML model to see how
              behaviour changes affect the churn risk score in real-time.
            </p>
            ...
            >
              {[
                {
                  key: "engagement_delta_7d",
                  label: "Engagement Delta (7d)",
                  min: -1,
                  max: 1,
                  step: 0.01,
                  desc: "Change in engagement events vs prior week. Negative = decline.",
                },
                {
                  key: "negative_event_ratio",
                  label: "Negative Event Ratio",
                  min: 0,
                  max: 1,
                  step: 0.01,
                  desc: "Proportion of engagement events that are drops/failures (0–1).",
                },
                {
                  ...
                  max: 90,
                  step: 1,
                  desc: "Days elapsed since last platform interaction.",
                },
                {
                  key: "contract_weight_index",
                  label: "Contract Value Weight",
                  min: 0,
                  max: 3,
                  step: 0.01,
                  desc: "Normalised contract value multiplier (1.0 = average).",
                },
                ...
                return (
                  <div key={key}>
                    ...
                    >
                      <label style={{ fontSize: "13px", marginBottom: 0 }}>
                        {label}
                      </label>
                      <strong
      ...
      <div className="card" style={{ padding: "0", overflow: "hidden" }}>
        <SectionHeader
          id="feed"
          title="Unified Activity Feed & CRM Notes"
          ...
          >
            ...
            >
              ...
              {noteSuccess && (
                ...
                >
                  ✓ Note saved and added to the feed.
                </p>
              )}
            </div>
                ...
                activityFeed.map((item) => {
                  const typeStyles = {
                    note: {
                      ...
                      border: "hsl(var(--hue), 75%, 85%)",
                      icon: (
                        <MessageSquarePlus size={13} color="var(--primary)" />
                      ),
                    },
                    intervention: {
                      bg: "var(--warning-bg)",
                      border: "hsl(38,92%,85%)",
                      icon: <Bell size={13} color="var(--warning)" />,
                    },
                    event: {
                      bg: "var(--bg-elevated)",
                      border: "var(--border)",
                      icon: <Activity size={13} color="var(--text-muted)" />,
                    },
                  };
